...
function SortableTaskCard({
  ...
  if (editing) {
    return (
      <div ref={setNodeRef} className="rounded-xl border border-blue-500/30 bg-[var(--bg-surface)] p-3 space-y-2 shadow-sm">
        <input
          autoFocus
          value={editTitle}
          onChange={e => setEditTitle(e.target.value)}
          className="w-full rounded border border-white/10 bg-white/5 px-2 py-1 text-sm text-[var(--text-primary)] outline-none focus:border-blue-400"
          placeholder="Título"
        />
        <input
          value={editDesc}
          onChange={e => setEditDesc(e.target.value)}
          className="w-full rounded border border-white/10 bg-white/5 px-2 py-1 text-xs text-[var(--text-primary)] outline-none focus:border-blue-400"
          placeholder="Descripción"
        />
        <select
          value={editPriority}
          onChange={e => setEditPriority(e.target.value as TaskPriority)}
          className="w-full rounded border border-white/10 bg-white/5 px-2 py-1 text-xs text-[var(--text-primary)] outline-none"
        >
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setEditing(false)} className="flex-1 rounded bg-white/10 py-1 text-xs font-semibold text-[var(--text-muted)] hover:bg-white/15 transition">Cancelar</button>
        </div>
      </div>
    )
  }
    ...
    >
      <div className={`absolute top-1.5 right-1.5 z-10 items-center gap-0.5 ${isDragging ? 'hidden' : 'hidden group-hover:flex'}`}>
        <button
          ...
          className="rounded p-1 bg-[var(--bg-surface)]/90 text-[var(--text-muted)] hover:text-blue-400 hover:bg-blue-500/10 transition shadow-sm"
          title="Editar"
        >
        ...
        <button
          ...
          className="rounded p-1 bg-[var(--bg-surface)]/90 text-[var(--text-muted)] hover:text-red-400 hover:bg-red-500/10 transition shadow-sm"
          title="Eliminar"
        >
        ...
        <div className="flex flex-col gap-1.5 px-2 pb-2 min-h-[4rem]">
          <AnimatePresence>
            {hasRealTasks ? columnTasks.map(t => (
              ...
              <div className="py-3 text-center text-xs text-[var(--text-muted)]">Sin tareas</div>
            )}
          </AnimatePresence>
