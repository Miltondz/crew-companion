...
Copy the Generative-UI Hackathon Starter Kit structure into this repo, remove the leads/Notion domain, and create route placeholders.

- [ ] Copy `apps/`, `deployment/`, `scripts/` from starter kit
- [ ] Remove leads domain (components, pages, Python modules)
- [ ] Create placeholder pages for `/leader`, `/member/[memberId]`, `/docs`
...
Build the complete frontend state layer for the crew domain.

- [ ] `apps/frontend/src/lib/crew/types.ts` — all TypeScript interfaces
- [ ] `apps/frontend/src/lib/crew/derive.ts` — `getUrgencyPhase`, `getMascotMood` pure functions
- [ ] `apps/frontend/src/lib/crew/seed.ts` — demo data with dynamic deadline
- [ ] `apps/frontend/src/lib/crew/store.ts` — Zustand store with all actions including `simulateUrgency`
- [ ] `apps/frontend/src/hooks/use-urgency-phase.ts` — reactive phase sync (30s interval)
...
Implement the three main views with CopilotKit integration and urgency-aware UI.

- [ ] `CopilotKitProviderShell` adapted for crew-companion
- [ ] `/leader` — TaskBoard, MilestonePanel, TeamOverview, `useCopilotReadable` for leader context
- [ ] `/member/[memberId]` — ActiveTaskView, MilestoneCountdown, BlockerButton, `useCopilotReadable` for member context
- [ ] `/docs` — DocumentTabs, MarkdownViewer with `rehype-sanitize`
- [ ] `UrgencyBanner` component (phase-aware color system)
- [ ] All 7 `useCopilotAction` frontend actions registered
- [ ] Dev-only urgency simulation buttons (`simulateUrgency`)
...
- [ ] `SurfaceRenderer` dispatcher component
- [ ] `renderSurface` CopilotKit action registered on all pages
- [ ] 8 MVP surfaces implemented (lead developer + collaborator):
  - [ ] `TaskSuggestionPanel`
...
- [ ] `MilestoneCountdown` — live 1-second countdown with phase-aware styling
- [ ] `CompanionMascot` — mood-reactive mascot component
