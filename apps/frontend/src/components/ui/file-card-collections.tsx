...
export const VALID_FORMATS: FormatFileProps[] = [
  "doc","pdf","md","mdx","csv","xls","xlsx","txt","ppt","pptx",
  "zip","rar","tar","gz","code","html","js","jsx","tsx","css",
  "json","img","png","jpg","jpeg","video",
];

export function inferFileFormat(filename: string): FormatFileProps {
  const ext = filename.split('.').pop()?.toLowerCase() ?? ''
  return VALID_FORMATS.includes(ext as FormatFileProps)
    ? (ext as FormatFileProps)
    : 'doc'
}
...
export const FileCard = ({ formatFile }: FileCardProps) => {
          ...
          {[3, 3, 2, 1].map((cols, i) => (
            <div key={i} className="grid grid-cols-3 gap-0.5">
              {Array.from({ length: cols }).map((_, j) => (
                <div key={j} className="bg-foreground/5 h-1 rounded-full" />
              ))}
            </div>
          ))}
        </div>
    ...
    filePlaceholder = (
      <>
        <div className="bg-foreground/5 mb-1.5 space-y-1 rounded border p-1">
          <div className="flex justify-center"><div className="size-3 rounded-sm bg-orange-400/40" /></div>
  ...
  if (["img","png","jpg","jpeg"].includes(formatFile)) {
    filePlaceholder = (
      <div className="bg-foreground/5 mb-1.5 space-y-1 rounded border p-1">
        <div className="flex justify-center"><div className="size-3 rounded-sm bg-yellow-400/40" /></div>
  ...
  if (formatFile === "css") {
    filePlaceholder = (
      <div className="space-y-1">
        ...
        {[["w-3","w-4"],["w-4","w-2"],["w-3","w-4"]].map(([a, b], i) => (
          <div key={i} className="flex items-center gap-1 pl-1.5">
            <div className={cn("h-0.75 rounded-full bg-sky-400/60", a)} />
            <div className={cn("h-0.75 rounded-full bg-sky-400/60", b)} />
          </div>
        ))}
        <div className="text-foreground/40 font-mono text-[6px]">{"}"}</div>
      </div>
  ...
  if (formatFile === "json") {
    filePlaceholder = (
      <div className="space-y-1">
        <div className="text-foreground/40 font-mono text-[6px]">{"{"}</div>
        {[["w-3","w-4","bg-foreground/20"],["w-4","w-2","bg-foreground/10"],["w-3","w-4","bg-foreground/10"],["w-3","","bg-foreground/10"]].map(([a, b, c], i) => (
          <div key={i} className="flex items-center gap-1 pl-1.5">
            <div className={cn("h-0.75 rounded-full", c, a)} />
            {b && <div className={cn("h-0.75 rounded-full", c, b)} />}
          </div>
        ))}
        <div className="text-foreground/40 font-mono text-[6px]">{"}"}</div>
      </div>
  ...
  return (
    <div aria-hidden className="relative size-fit">
      ...
      <div className={cn(
        "dark:bg-secondary ring-border relative z-[1] w-14 space-y-3 rounded-md bg-white p-2 ring-1",
        "h-[72px]"
      )}>
        {filePlaceholder}
      </div>
